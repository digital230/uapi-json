...
const searchLowFaresValidate = obj => {
  ...
  if (
    Object.prototype.toString.call(obj["air:AirPricePointList"]) !==
      "[object Object]" &&
    Object.prototype.toString.call(obj["air:AirPricingSolution"]) !==
      "[object Object]"
  ) {
    throw new AirParsingError.ResponseDataMissing({
      missing: "AirPricePoint or AirPricingSolution"
    });
  }
  ...
  if (obj["air:ETR"]) {
    try {
      checkTickets = Object.values(obj["air:ETR"]).reduce((acc, x) => {
        const tickets = Object.values(x["air:Ticket"] || {}).reduce(
          (acc2, t) => !!(acc2 && t.TicketNumber),
          true
        );
        return !!(acc && tickets);
      }, true);
...
function airFlightInfoRsp(obj) {
  const data = this.mergeLeafRecursive(obj, "air:FlightInformationRsp")[
    "air:FlightInfo"
  ];
...
function airPriceRspPassengersPerReservation(obj) {
  const data = this.mergeLeafRecursive(obj, "air:AirPriceRsp")[
    "air:AirPriceRsp"
  ];
  ...
  return Object.keys(pricing).reduce(
    (acc, right) => ({
      ...acc,
...
function airPrice(obj) {
  ...
  if (priceKeys.length > 1) {
    console.log(
      "More than one solution found in booking. Resolving the cheapest one."
    );
    const solutions = priceKeys.map(key => pricingSolutions[key]);

    [pricingSolution] = solutions.sort(
      (a, b) =>
        parseFloat(a.TotalPrice.slice(3)) - parseFloat(b.TotalPrice.slice(3))
    );
  } else {
  ...
  // console.log("airSegments", airSegments);
  ...
  const directions = groups.map(leg => {
    ...
    const trips = segs.map(segment => {
      const tripFlightDetails = Object.keys(segment["air:FlightDetails"]).map(
        flightDetailsRef => segment["air:FlightDetails"][flightDetailsRef]
      );
      // console.log('segment["air:FlightDetails"]', tripFlightDetails);

      const [bookingInfo] = thisFare["air:BookingInfo"].filter(
        info => info.SegmentRef === segment.Key
      );
      ...
      // console.log('tripFlightDetails["air:Meals"]', tripFlightDetails[0]);

      return Object.assign(format.formatTrip(segment, tripFlightDetails), {
        serviceClass: bookingInfo.CabinClass,
        bookingClass: bookingInfo.BookingCode,
        fareBasisCode: fareInfo.FareBasis,
        baggage,
        entertainment: {
          meal: tripFlightDetails[0]["air:Meals"],
          InFlightServices: tripFlightDetails[0]["air:InFlightServices"]
        }
      });
    });
  ...
  const { passengerCounts, passengerFares } = format.formatPassengerCategories(
  ...
  const taxesInfo = thisFare["air:TaxInfo"]
    ? Object.keys(thisFare["air:TaxInfo"]).map(taxKey =>
        Object.assign(
          {
            value: thisFare["air:TaxInfo"][taxKey].Amount,
            type: thisFare["air:TaxInfo"][taxKey].Category
          },
          thisFare["air:TaxInfo"][taxKey][
            `common_${this.uapi_version}:TaxDetail`
          ]
            ? {
                details: thisFare["air:TaxInfo"][taxKey][
                  `common_${this.uapi_version}:TaxDetail`
                ].map(taxDetail => ({
...
function airPriceRspPricingSolutionXML(obj) {
  ...
  const passengersPerReservations = airPriceRspPassengersPerReservation.call(
    this,
    objCopy
  );

  const segments =
    obj["air:AirPriceRsp"][0]["air:AirItinerary"][0]["air:AirSegment"];
  ...
  if (pricingSolutions.length > 1) {
    ...
    [pricingSolution] = pricingSolutions.sort(
      (a, b) =>
        parseFloat(a.$.TotalPrice.slice(3)) -
        parseFloat(b.$.TotalPrice.slice(3))
    );
  } else {
  ...
  const pricingInfos = pricingSolution["air:AirPricingInfo"].map(info =>
    Object.assign({}, info, { "air:PassengerType": [] })
  );
  ...
  this.env.passengers.forEach((passenger, index) => {
    ...
    const pricingInfo = pricingInfos.find(
      info => info.$.Key === reservationKey
    );
  ...
  [
    ...
    "air:AirPricingInfo",
    "air:FareNote",
    `common_${this.uapi_version}:HostToken`
  ].forEach(root => {
  ...
  const mergedSegments = this.mergeLeafRecursive(objCopy, "air:AirPriceRsp")[
    "air:AirPriceRsp"
  ]["air:AirItinerary"]["air:AirSegment"];
...
const AirErrorHandler = function(rsp) {
  ...
  try {
    ...
    throw new RequestRuntimeError.UnhandledError(
      null,
      new AirRuntimeError(rsp)
    );
  }
  switch (code) {
    case "345":
    ...
    case "3000": {
      const messages = errorInfo["air:AirSegmentError"].map(
        err => err["air:ErrorMessage"]
      );
      if (
        messages.indexOf(
          "Booking is not complete due to waitlisted segment"
        ) !== -1
      ) {
...
function getTicketFromEtr(etr, obj) {
  ...
  const passengers = Object.keys(passengersList).map(passengerKey => {
    const travelerDetails =
      passengersList[passengerKey][
        `common_${this.uapi_version}:BookingTravelerName`
      ];
    const firstName = travelerDetails.First.concat(
      travelerDetails.Prefix || ""
    );
  ...
  const fareInfo =
    airPricingInfo && airPricingInfo["air:FareInfo"]
  ...
  const tickets = ticketsList.map(ticket => {
    if (ticket["air:ExchangedTicketInfo"]) {
      ticket["air:ExchangedTicketInfo"].forEach(t =>
        exchangedTickets.push(t.Number)
      );
    }
    ...
    const coupons = Object.keys(ticket["air:Coupon"]).map(couponKey => {
      ...
      if (airPricingInfo && airPricingInfo["air:FareInfo"]) {
        Object.keys(airPricingInfo["air:FareInfo"]).forEach(fareKey => {
          ...
          if (
            fare.FareBasis === coupon.FareBasis &&
      ...
      return {
        ...coupon,
        stopover: nextCoupon ? nextCoupon.stopover : true,
        ...(bookingInfo !== null
          ? { serviceClass: bookingInfo.CabinClass }
          : null)
      };
  ...
  const taxes =
    airPricingInfo && airPricingInfo["air:TaxInfo"]
      ? Object.keys(airPricingInfo["air:TaxInfo"]).map(taxKey =>
          Object.assign(
            {
              type: airPricingInfo["air:TaxInfo"][taxKey].Category,
              value: airPricingInfo["air:TaxInfo"][taxKey].Amount
            },
            airPricingInfo["air:TaxInfo"][taxKey][
              `common_${this.uapi_version}:TaxDetail`
            ]
              ? {
                  details: airPricingInfo["air:TaxInfo"][taxKey][
                    `common_${this.uapi_version}:TaxDetail`
                  ].map(taxDetail => ({
                    airport: taxDetail.OriginAirport,
                    value: taxDetail.Amount
  ...
  const commission =
    (etr && etr[`common_${this.uapi_version}:Commission`]) ||
    ...
    priceInfoAvailable
      ? {
          totalPrice:
            ...
            `${(priceSource.EquivalentBasePrice || priceSource.BasePrice).slice(
              0,
              3
            )}0`,
      ...
      : null,
    exchangedTickets.length > 0 ? { exchangedTickets } : null
  );
...
const airGetTicket = function(obj) {
  ...
  if (multipleTickets) {
    return Object.values(etr).map(innerEtr =>
      getTicketFromEtr.call(this, innerEtr, obj)
    );
  }
...
function extractBookings(obj) {
  ...
  if (
    ...
    record["air:AirReservation"].length === 0
  ) {
  ...
  const remarks = remarksObj
    ? Object.keys(remarksObj).reduce((acc, key) => {
  ...
  return record["air:AirReservation"].map(booking => {
    ...
    const splitBookings =
      ...
      providerInfo["universal:ProviderReservationDetails"].DivideDetails ===
        "true"
        ? resRemarks.reduce((acc, remark) => {
            const splitMatch = remark["common_v47_0:RemarkData"].match(
              /^SPLIT\s.*([A-Z0-9]{6})$/
            );
            if (!splitMatch) {
              return acc;
    ...
    const passiveReservation = record["passive:PassiveReservation"]
      ? record["passive:PassiveReservation"].find(
          res => res.ProviderReservationInfoRef === providerInfoKey
        )
      : null;
    ...
    const passengers = booking[
      `common_${this.uapi_version}:BookingTravelerRef`
    ].map(travellerRef => {
    ...
    const {
      ...
      (passiveReservation && passiveReservation["passive:PassiveSegment"]) ||
        null
    );

    const supplierLocator =
      booking[`common_${this.uapi_version}:SupplierLocator`] || [];
      ...
      ? indexedServiceSegments
          .map(s => {
            ...
            try {
              ...
              console.warn(
                `PassiveRemark is not service segment: ${
                  remark["passive:Text"]
                }.`
              );
    ...
    const tickets =
      booking["air:DocumentInfo"] &&
      booking["air:DocumentInfo"]["air:TicketInfo"]
    ...
    const pricingInfos = !booking["air:AirPricingInfo"]
      ? []
      : Object.keys(booking["air:AirPricingInfo"])
          .map(key => {
            ...
            const uapiPassengerRefs =
              pricingInfo[`common_${this.uapi_version}:BookingTravelerRef`];

            const fareInfo = pricingInfo["air:FareInfo"];

            const baggage =
              fareInfo &&
              Object.keys(fareInfo).map(fareLegKey =>
                format.getBaggage(fareInfo[fareLegKey]["air:BaggageAllowance"])
              );
            ...
            const taxesInfo = pricingInfo["air:TaxInfo"]
              ? Object.keys(pricingInfo["air:TaxInfo"]).map(taxKey =>
                  Object.assign(
                    {
                      value: pricingInfo["air:TaxInfo"][taxKey].Amount,
                      type: pricingInfo["air:TaxInfo"][taxKey].Category
                    },
                    pricingInfo["air:TaxInfo"][taxKey][
                      `common_${this.uapi_version}:TaxDetail`
                    ]
                      ? {
                          details: pricingInfo["air:TaxInfo"][taxKey][
                            `common_${this.uapi_version}:TaxDetail`
                          ].map(taxDetail => ({
                            airport: taxDetail.OriginAirport,
                            value: taxDetail.Amount
            ...
            const pricingInfoPassengers = (uapiPassengerRefs || []).map(ref => {
              const ticket = tickets.find(
                t =>
                  t.uapi_passenger_ref === ref &&
                  t.uapi_pricing_info_ref === key
              );
              return Object.assign(
                {
    ...
    const fareQuotesGrouped = pricingInfos.reduce(
      (acc, pricingInfo) =>
        Object.assign(acc, {
          [pricingInfo.uapi_pricing_info_group]: (
            acc[pricingInfo.uapi_pricing_info_group] || []
          ).concat(pricingInfo)
        }),
      {}
    );
    ...
    const fareQuotes = Object.keys(fareQuotesGrouped)
      .map(key => {
        ...
        const fqGroupPassengers = fqGroup.reduce(
          (acc, fq) => acc.concat(fq.passengers.map(p => p.uapi_passenger_ref)),
          []
        );
      ...
      .sort((fq1, fq2) => moment(fq1.effectiveDate) - moment(fq2.effectiveDate))
      .map((fq, index) => ({ ...fq, index: index + 1 }));
    ...
    return Object.assign(
      ...
      splitBookings.length > 0 ? { splitBookings } : null,
...
function extractFareRules(obj) {
  ...
  rulesList.forEach(item => {
    const result = [];
    const listName = item["air:FareRuleLong"]
      ? "air:FareRuleLong"
      : "air:FareRuleShort";
...
function exchangeQuote(req) {
  ...
  return utils.deflate(JSON.stringify(token)).then(zippedToken => {
    ...
    const totalBundle =
      exchangeBundleTotal && exchangeBundleTotal[exchangeInfoRoot];

    const exchangeDetails = json["air:AirExchangeBundle"].map(bundle => {
      const exchange = bundle[exchangeInfoRoot];
      const taxes = exchange[`common_${this.uapi_version}:PaidTax`].map(
        tax => ({ type: tax.Code, value: tax.Amount })
      );
    ...
    const pricingInfo = Object.keys(solution["air:AirPricingInfo"]).map(key => {
      ...
      const bookingInfo = Object.keys(pricing["air:BookingInfo"]).map(
        bookingInfoKey => {
...
function availability(rsp) {
  ...
  const connectedSegments = itinerarySolution["air:Connection"]
    ? itinerarySolution["air:Connection"].map(s => parseInt(s.SegmentIndex, 10))
    : [];
  const results = [];
  let leg = [];
  itinerarySolution["air:AirSegmentRef"].forEach((segmentRef, key) => {
    ...
    const availInfo = segment["air:AirAvailInfo"].find(
      info => info.ProviderCode === this.provider
    );
    ...
    const cabinsAvailability = availInfo["air:BookingCodeInfo"]
      ...
      .reduce((acc, x) => {
        const codes = x.BookingCounts.split("|").map(item => ({
          bookingClass: item[0],
          cabin: x.CabinClass,
          seats: item[1].trim()
        }));
  ...
  return {
    legs: results,
    nextResultReference:
      rsp[`common_${this.uapi_version}:NextResultReference`] || null
  };
}
