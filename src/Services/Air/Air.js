...
module.exports = settings => {
  ...
  return {
    ...
    getPNR(options) {
      console.warn(
        "DEPRECATED, will be dropped in next major version, use getBooking"
      );
    ...
    getUniversalRecordByPNR(options) {
      return service.getUniversalRecordByPNR(options).catch(err => {
        ...
        const pnrRegExp = new RegExp(
          `^(?:\\*\\* THIS BF IS CURRENTLY IN USE \\*\\*\\s*)?${options.pnr}`
        );
        return terminal
          .executeCommand(`*${options.pnr}`)
          .then(response => {
            if (!response.match(pnrRegExp)) {
              return Promise.reject(
                new AirRuntimeError.UnableToOpenPNRInTerminal()
              );
            }
          ...
          .then(response => {
            if (response.indexOf(segmentResult) === -1) {
              return Promise.reject(
                new AirRuntimeError.UnableToAddExtraSegment()
              );
            }
            return Promise.resolve();
          })
          .then(() => {
            const ticketingDate = moment()
              .add(10, "days")
              .format("DDMMM");
          ...
          .then(response => {
            if (
              ...
              return Promise.reject(
                new AirRuntimeError.UnableToSaveBookingWithExtraSegment()
              );
            }
            return Promise.resolve();
          })
    ...
    ticket(options) {
      return (options.ReservationLocator
        ...
        : this.getBooking(options).then(
            booking => booking.uapi_reservation_locator
          )
      ).then(ReservationLocator =>
        retry({ retries: 3 }, (again, number) => {
    ...
    getTickets(options) {
      return service
        ...
        .catch(err =>
          Promise.reject(
            new AirRuntimeError.UnableToRetrieveTickets(options, err)
          )
        );
    },
    ...
    getBookingByTicketNumber(options) {
      const terminal = createTerminalService(settings);
      return terminal
        .executeCommand(`*TE/${options.ticketNumber}`)
        .then(response =>
          terminal
            .closeSession()
            .then(() => response.match(/RLOC [^\s]{2} ([^\s]{6})/)[1])
            .catch(() =>
              Promise.reject(new AirRuntimeError.PnrParseError(response))
            )
        )
        .catch(err =>
          Promise.reject(new AirRuntimeError.GetPnrError(options, err))
        );
    },

    getPNRByTicketNumber(options) {
      console.warn(
        "DEPRECATED, will be dropped in next major version, use getBookingByTicketNumber"
      );
    ...
    cancelTicket(options) {
      return this.getTicket(options)
        ...
        .catch(err =>
          Promise.reject(new AirRuntimeError.FailedToCancelTicket(options, err))
        );
    },

    cancelBooking(options) {
      const ignoreTickets =
        typeof options.ignoreTickets === "undefined"
          ? false // default value
      ...
      const checkTickets = tickets => {
        return Promise.all(
          tickets.map(ticketData => {
            ...
            const allTicketsVoidOrRefund = ticketData.tickets.every(ticket =>
              ticket.coupons.every(
            ...
            const hasNotOpenSegment = ticketData.tickets.some(ticket =>
              ticket.coupons.some(coupon => "OV".indexOf(coupon.status) === -1)
            );
            if (hasNotOpenSegment) {
              return Promise.reject(
                new AirRuntimeError.UnableToCancelTicketStatusNotOpen()
              );
            }
            return Promise.all(
      ...
      return this.getUniversalRecordByPNR(options).then(ur => {
        ...
        return (ignoreTickets
          ...
          .catch(err =>
            Promise.reject(new AirRuntimeError.FailedToCancelPnr(options, err))
          );
      });
    },
    ...
    cancelPNR(options) {
      console.warn(
        "DEPRECATED, will be dropped in next major version, use cancelBooking"
      );
      return this.cancelBooking(options);
    },

    getExchangeInformation(options) {
      return this.getBooking(options).then(booking =>
        service.exchangeQuote({
          ...options,
